...
void borrarPelicula(ePelicula lista[],int tam)
{
    ...
    mostrarPeliculasConId(lista,tam);
    ...
    index=buscarId(lista,tam,id);
        if(index!=-1)
        {
            ...
            validacion=pedirChar("Es la pelicula que desea borrar? ");
            validarCharSeguir(validacion);
            if(validacion=='s')
            {
...
void mostrarPeliculasCompletas(ePelicula listaPeliculas[],int tam,eGenero listaGeneros[],int tg,eActor listaActores[],int ta)
{
    int i;
        ...
        for(i=0;i<tam;i++)
        {
            ...
            {
              mostrarUnaPeliculaCompleta(listaPeliculas[i],listaGeneros,tg,listaActores,ta);
            }
        }
}
void mostrarUnaPeliculaCompleta(ePelicula pelicula,eGenero listaGeneros[],int tg,eActor listaActores[],int ta)
{
    ...
    eActor auxActor;
        ...
        for(i=0;i<tg;i++)
        {
            if(pelicula.genero==listaGeneros[i].id)
            {
                auxGenero=listaGeneros[i];
                break;
            }
        }
        ...
        for(j=0;j<ta;j++)
        {
            if(pelicula.actor==listaActores[j].id)
            {
                auxActor=listaActores[j];
            }
        }

        printf("%25s:  %2d/%2d/%4d  %8s  %15s  %9s \n",pelicula.titulo,pelicula.estreno.dia,pelicula.estreno.mes,pelicula.estreno.anio,auxGenero.descripcion,auxActor.nombre,auxActor.pais);
}
...
void mostrarPeliculasActorEEUU(ePelicula listaPeliculas[],int tam, eActor listaActores[],int ta)
{
    int i;
    int j;
    printf("Peliculas con actores de EEUU: \n\n");
        for(i=0;i<tam;i++)
        {
            if(listaPeliculas[i].estado==OCUPADO)
            {
                for(j=0;j<ta;j++)
                {
                    if(listaPeliculas[i].actor==listaActores[j].id&&strcmp(listaActores[j].pais,"EEUU")==0)
                    {
    ...
    int j;
        for(i=0;i<tg;i++)
        {
            printf("%s: \n",listaGeneros[i].descripcion);
                for(j=0;j<tam;j++)
                {
                    if(listaPeliculas[j].genero==listaGeneros[i].id)
                    {
...
void contarPeliculasPorGenero(ePelicula listaPeliculas[],int tam,eGenero listaGeneros[],int tg)
{
    int i;
    int j;
    int cantPeliculas=0;
        for(i=0;i<tg;i++)
        {
            printf("%s: ",listaGeneros[i].descripcion);
                for(j=0;j<tam;j++)
                {
                    if(listaPeliculas[j].genero==listaGeneros[i].id)
                    {
...
void mostrarGeneroMenosPeliculas(ePelicula listaPeliculas[],int tam,eGenero listaGeneros[],int tg)
{
    ...
    for(i=0;i<tg;i++)
    {
        ...
        if(flag==0||cantPeliculas<peliculasMin)
        {
            strcpy(generoMin,listaGeneros[i].descripcion);
...
void mostrarPeliculasConId(ePelicula listaPeliculas[],int tam)
{
    int i;
    for(i=0;i<tam;i++)
    {
        if(listaPeliculas[i].estado==OCUPADO)
        {
        printf("%25s, id: %4d\n",listaPeliculas[i].titulo,listaPeliculas[i].id);
        }
    }
}
